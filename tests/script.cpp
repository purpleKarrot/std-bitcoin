...
TEST_CASE("script copies bytes and can represent oversized scripts")
{
  ...
  auto ref = bitcoin::script_ref{script};
  CHECK_FALSE(ref.empty());
  CHECK(as_bytes(ref).size() == bytes.size());
  CHECK(std::ranges::equal(as_bytes(ref), std::span{bytes}));
  CHECK(ref == script);

  auto copied = bitcoin::script{ref};
  CHECK(copied == script);

  auto oversized = std::vector<std::byte>(script_size_limit + 1, byte(0x00));
  auto oversized_script = bitcoin::script{std::span{oversized}};
...
TEST_CASE("instruction views decode opcodes and pushed bytes")
{
  ...
  for (auto instruction : bitcoin::instructions(ref)) {
...
{
  auto truncated_push_bytes = std::array{byte(0x02), byte(0xaa)};
  auto truncated_push = bitcoin::script{std::span{truncated_push_bytes}};
  ...
  auto undefined_opcode_bytes = std::array{byte(0xbb)};
  auto undefined_opcode = bitcoin::script{std::span{undefined_opcode_bytes}};
...
TEST_CASE("witness-program and common output templates are recognized")
{
  ...
  auto p2sh = bitcoin::script{std::span{p2sh_bytes}};
  ...
  auto p2wsh = bitcoin::script{std::span{p2wsh_bytes}};

  auto program = bitcoin::witness_program(p2wsh);
  ...
  auto p2tr = bitcoin::script{std::span{p2tr_bytes}};

  auto taproot_program = bitcoin::witness_program(p2tr);
...
{
  auto suffix_bytes = std::array{byte(bitcoin::opcode::op_equal)};
  ...
  auto payload = std::vector<std::byte>(script_size_limit, byte(0x99));
