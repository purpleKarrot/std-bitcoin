...
verification_status validate(block_header const& header,
                             bitcoin::any_chain chain,
                             std::chrono::sys_seconds time);

verification_status validate(bitcoin::transaction const& tx);

verification_status validate(bitcoin::block const& block,
                             bitcoin::any_chain chain,
                             utxo_access const& utxo_set,
                             std::chrono::sys_seconds time);
...
verification_status verify(
  ...
  beman::any_view::any_view<tx_output const,
    ...
    spent_outputs = {},
  verification_flags flags = verification_flags::all);
